...
@Repository
public interface LicenseAssignmentFactRepository extends JpaRepository<LicenseAssignmentFact, String> {


    @Query("""
                SELECT
                    new net.pvytykac.modularapp.analytics.internal.fact.license.LicenseAssignmentBreakdown(
                        date(la.assignmentTime) as date,
                        SUM(case when (la.eventType = ASSIGNED) THEN 1 ELSE 0 END) as assignedCount,
                        SUM(case when (la.eventType = ASSIGNED) THEN la.licenseMonthlyPrice ELSE 0 END) as assignedCost,
    ...
    @Query(value = """
                SELECT
                    new net.pvytykac.modularapp.analytics.internal.fact.license.LicenseAssignmentStatistics(
                        dimLic.licenseId,
                        dimLic.name,
                        dimLic.cost,
                        SUM(case when (la.eventType = ASSIGNED) THEN 1 ELSE 0 END) as assignedCount,
